...
CREATE TABLE `fy_dictionary_room` (
  ...
  `room_is_valid` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '是否有效{0:无效,1:有效}',
  ...
  `room_with_id` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '朝向ID{1:东,2:南,3:西,4:北}',
  ...
  `room_basic_property` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '是否有产权{1:有,0:无}',
  ...
  `room_basic_number_level` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '号码等级{1:A,2:B,3:C}',
  ...
  `room_modify_time` timestamp NOT NULL DEFAULT '0000-00-00 00:00:00' COMMENT '最后一次修改时间',
  `room_quality` tinyint(1) unsigned NOT NULL COMMENT '产权性质{1:商品房,2:房改房,3:拆迁安置房,4:安居房,5:公房,6:经济适用房,7:私房}',
  `audit_status` smallint(1) unsigned NOT NULL DEFAULT '0' COMMENT '审核状态',
